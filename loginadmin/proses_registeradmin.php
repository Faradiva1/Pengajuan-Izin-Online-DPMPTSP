...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $new_username = $_POST['new_username_admin'];
    $new_password = $_POST['new_password_admin'];
    $confirm_password = $_POST['confirm_password'];
    $role = "admin"; // Atur default role sebagai "user"

    // Validasi password
    if ($new_password !== $confirm_password) {
        header("Location: adminpageregister.php?error=1");
        exit();
    }
    ...
    if ($check_stmt->num_rows > 0) {
        header("Location: adminpageregister.php?error=2");
        exit();
    }
    ...
    if ($insert_stmt->execute()) {
        header("Location: adminpageregister.php?success=1");
        exit();
    } else {
        header("Location: adminpageregister.php?error=3");
        exit();
    }
}
